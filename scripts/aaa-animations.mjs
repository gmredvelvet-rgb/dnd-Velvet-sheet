...
const VA = {
  ...
  sheetEnter(html) {
    ...
    g.set(sheet.querySelector(".aaa-sidebar"),   { x: -28, opacity: 0 });
    g.set(sheet.querySelector(".center"),           { scale: 0.96, opacity: 0 });
    g.set(sheet.querySelector(".aaa-panel"),     { x: 28, opacity: 0 });
    ...
    tl
      .to(sheet.querySelector(".aaa-sidebar"),  { x: 0, opacity: 1, duration: 0.45 })
      .to(sheet.querySelector(".center"),          { scale: 1, opacity: 1, duration: 0.4 }, "-=0.30")
      .to(sheet.querySelector(".aaa-panel"),    { x: 0, opacity: 1, duration: 0.45 }, "-=0.35")
   ...
   * @param {Element} panelEl  â€” the tab content element that was just made visible
   * @param {number}  dir      â€” +1 = going right / forward, -1 = going left / back
   */
   ...
   * @param {jQuery} html
   * @param {number} newPct   â€” 0-100
   * @param {number} delta    â€” negative = damage, positive = healing
   */
  ...
  /**
   * Called when dying value increases â€” dramatic red flash.
   * @param {jQuery} html
   */
  dyingIncreased(html) {
    const g = this.gsap;
  ...
  _floatNumber(html, delta) {
    ...
    el.className = "aaa-float-num " + (delta < 0 ? "aaa-float-dmg" : "aaa-float-heal");
  ...
  _startAmbientParticles(html) {
    ...
    const spawn = () => {
      ...
      if (container.querySelectorAll(".aaa-ember").length >= 14) return;
      const p = document.createElement("div");
      p.className = "aaa-ember";
