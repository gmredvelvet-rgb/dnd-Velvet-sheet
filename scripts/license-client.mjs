...
const OWN_MODULE_ID = 'dnd-velvet-sheets';
...
export class AAALicenseClient {
  ...
  static get instance() {
    if (!this.#instance) this.#instance = new AAALicenseClient();
  ...
  async activateWithCode(authCode) {
    ...
    this.#startHeartbeat();
    Hooks.callAll('aaa-sheet.activate');
    game.settings?.set?.(OWN_MODULE_ID, 'worldLicensed', true).catch(() => {});
    AAALicenseUI.stopReminder();
    ui.notifications?.info(`AAA D&D Sheet: Connected as ${result.tier} subscriber. Welcome!`);
  }
  ...
  async releaseInstallation() {
    try {
      ...
      ui.notifications?.info('AAA D&D Sheet: Installation slot released.');
    } catch (e) {
      ui.notifications?.error(`AAA D&D Sheet: Failed to release slot — ${e.message}`);
    }
  }
  ...
  #isTransient(e) {
    if (!(e instanceof AAALicenseError)) return true;
  ...
  async #apiCall(endpoint, body, method = 'POST') {
    ...
    try {
      ...
      throw new AAALicenseError('License server unreachable', 'NETWORK_ERROR');
    }

    if (!resp.ok) {
      const err = await resp.json().catch(() => ({ error: 'Network error', code: 'NETWORK_ERROR' }));
      throw new AAALicenseError(err.error ?? 'Request failed', err.code ?? 'API_ERROR');
    }
  ...
  async #verifyResponseSig(payload, jwt) {
    ...
    if (parts.length !== 3) throw new AAALicenseError('Malformed response token', 'SIGNATURE_INVALID');
    ...
    if (!valid) throw new AAALicenseError('Server response verification failed', 'SIGNATURE_INVALID');

    const claims = JSON.parse(atob(body.replaceAll('-', '+').replaceAll('_', '/')));
    const now    = Math.floor(Date.now() / 1000);
    if (claims.exp && claims.exp < now) throw new AAALicenseError('Response token expired', 'REPLAY_DETECTED');
    ...
    if (claims.ph !== expectedPh) throw new AAALicenseError('Response payload tampered', 'SIGNATURE_INVALID');
  }

  async #verifyAndParseJwt(token) {
    const parts = token.split('.');
    if (parts.length !== 3) throw new AAALicenseError('Malformed JWT', 'INVALID_TOKEN');
    ...
    const valid = await crypto.subtle.verify('RSASSA-PKCS1-v1_5', key, sigBytes, data);
    if (!valid) throw new AAALicenseError('JWT signature invalid', 'INVALID_TOKEN');

    const payload = JSON.parse(atob(body.replaceAll('-', '+').replaceAll('_', '/')));
    const now     = Math.floor(Date.now() / 1000);
    if (payload.exp && payload.exp < now) throw new AAALicenseError('JWT expired', 'TOKEN_EXPIRED');
    return payload;
  }
}

export class AAALicenseError extends Error {
  constructor(message, code) {
    super(message);
    this.name = 'AAALicenseError';
    this.code = code;
  }
}

export class AAALicenseUI {
  ...
  static startReminder() {
    AAALicenseUI.#clearReminderTimer();
    if (isWorldLicensed()) return;
    AAALicenseUI.#reminderTimer = setInterval(() => {
      if (isWorldLicensed()) { AAALicenseUI.stopReminder(); return; }
      AAALicenseUI.show({ autoHide: true });
    }, REMINDER_MS);
  }

  static stopReminder() {
    AAALicenseUI.#clearReminderTimer();
    document.getElementById('aaa-sheet-license-prompt')?.remove();
  }

  static #clearReminderTimer() {
    if (AAALicenseUI.#reminderTimer) clearInterval(AAALicenseUI.#reminderTimer);
    AAALicenseUI.#reminderTimer = null;
  }

  static #cancelAutoHide() {
    if (AAALicenseUI.#hideTimer) clearTimeout(AAALicenseUI.#hideTimer);
    AAALicenseUI.#hideTimer = null;
  }

  static show({ autoHide = false } = {}) {
    if (!game.user?.isGM) return;

    const id = 'aaa-sheet-license-prompt';
    ...
    el.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
        ...
        <strong style="color:#c89b3c;font-size:1rem">AAA D&D Sheet</strong>
      </div>
      <p style="font-size:.85rem;color:#a09080;margin-bottom:16px;line-height:1.4">
        You're using AAA D&amp;D Sheet unlicensed — the full sheet is unlocked. If it's useful at your table, please support development by connecting your Patreon.
      </p>
      <div style="display:flex;flex-direction:column;gap:8px">
        <button id="aaa-connect-btn" style="
        ...
        <button id="aaa-dismiss-btn" style="
    ...
    el.querySelector('#aaa-connect-btn').addEventListener('click', async () => {
      AAALicenseUI.#cancelAutoHide();
      const btn = el.querySelector('#aaa-connect-btn');
      btn.textContent = 'Opening Patreon...';
      btn.disabled = true;
      try {
        const success = await AAALicenseClient.instance.startOAuth();
        if (success) { AAALicenseUI.stopReminder(); el.remove(); }
        else { btn.textContent = 'Connect Patreon'; btn.disabled = false; }
      } catch (e) {
        btn.textContent = 'Connect Patreon'; btn.disabled = false;
        ui.notifications?.error(`AAA D&D Sheet: ${e.message}`);
      }
    });

    el.querySelector('#aaa-code-btn').addEventListener('click', () => {
      AAALicenseUI.#cancelAutoHide();
      AAALicenseUI.showCodeInput();
      el.remove();
    });

    el.querySelector('#aaa-dismiss-btn').addEventListener('click', () => el.remove());
    el.addEventListener('pointerenter', () => AAALicenseUI.#cancelAutoHide());

    document.body.appendChild(el);

    if (autoHide) {
      AAALicenseUI.#hideTimer = setTimeout(() => {
        AAALicenseUI.#hideTimer = null;
        el.remove();
      }, AUTO_HIDE_MS);
    }
  ...
  static showCodeInput() {
    new Dialog({
      title: 'AAA D&D Sheet — Enter Auth Code',
      content: `
        ...
        <input id="aaa-auth-code-input" type="text"
      ...
      buttons: {
        activate: {
          ...
          callback: async (html) => {
            const code = html.find('#aaa-auth-code-input').val().trim();
            if (!code) return;
            try {
              await AAALicenseClient.instance.activateWithCode(code);
            } catch (e) {
              ui.notifications?.error(`AAA D&D Sheet: ${e.message}`);
            }
          }
